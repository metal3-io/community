...
- **Project Version:** v0.14.x (BMO), v1.14.x (CAPM3), v1.14.x (IPAM)
- **Website:** <https://metal3.io>
- **Date Updated:** 2026-09-17
- **Template Version:** v1.0
- **Description:** Metal3.io provides Kubernetes-native baremetal
  host provisioning and lifecycle management, integrated with the
  Cluster API for declarative baremetal cluster management. It
...
well-architected, Kubernetes-native solution for baremetal
...
The project fills a genuine gap in the Kubernetes ecosystem: declarative
baremetal management using standard BMC protocols
...
- Describe the target persona or user(s) for the project?
   - The project targets platform engineers and infrastructure
     operators managing baremetal Kubernetes clusters, primarily
...
- Explain the primary use case for the project. What additional
  use cases are supported by the project?
   - Declarative lifecycle management of baremetal servers as
     ...
     standalone baremetal management without CAPI via Baremetal
     Operator directly, IP address management for baremetal
     networks.
- Explain which use cases have been identified as unsupported by
  the project.
   - Metal3 does not manage cloud provider VMs or act as a
     general-purpose scheduler for containerized workloads
     (that remains the role of Kubernetes itself). The Metal3
     stack handles the lifecycle management of any physical
     computational resource as long as a compatible BMO
     provisioner interface implementation exists for that
     resource; hosts for which no such implementation exists
     (for example, hosts lacking supported BMC/out-of-band
     management access) are therefore out of scope. It is
     designed exclusively for bare metal "self-hosted"
     workflows and does not require any commercial cloud
     provider service.
...
- How should the target personas interact with your project?
   - Operators interact entirely through Kubernetes-native APIs
     ...
     Metal3Cluster, IPPool, IPClaim and other custom resources using
     standard tools like kubectl and clusterctl.
   - In Cluster API-based workflows, operators use clusterctl
     to initialize CAPM3 as the infrastructure provider, then
     declare Cluster, KubeadmControlPlane, and
     MachineDeployment resources. CAPM3 translates these into
     Metal3Cluster and Metal3Machine objects and drives bare
     metal lifecycle actions. Physical hosts must still be
     pre-registered as BareMetalHost resources (manually or
     via automation) before Metal3Machine reconciliation can
     claim them, but day-to-day provisioning is then driven
     through the Cluster API abstractions.
   - Some users interact with only the
     [Ironic Standalone Operator][irso], deploying and managing
     Ironic through its `Ironic` custom resource without
     adopting the rest of the Metal3 stack. This reflects
     Metal3's modular design, where IrSO can be consumed
     independently.
- Describe the user experience (UX) and user interface (UI) of
  the project.
   - Metal3 has no built-in graphical UI; the primary interface
     is the Kubernetes API with CRDs and status conditions. The
     Metal3 and Headlamp communities are collaborating on a
     [Metal3 Headlamp plugin][headlamp-plugin], which has an
     initial alpha release available and provides a
     Kubernetes-native UI for browsing and inspecting Metal3
     resources.
     Operators use kubectl (or other
   ...
   - The core experience is the BareMetalHost Custom Resource:
     ...
     the BMC address, a reference to a Kubernetes Secret
     holding the BMC credentials, and the desired state. No
     graphical dashboard
   ...
   - In Cluster API-based workflows, operators interact through
     ...
     (Metal3Cluster and Metal3Machine) and drives baremetal
     lifecycle actions.
- Describe how this project integrates with other projects in a
  production environment.
   - Metal3 implements the [Cluster API][capi] infrastructure provider
     contract for baremetal, integrates OpenStack Ironic
   ...
   - On the Ironic side, the integration surface includes the
     [Ironic Standalone Operator][irso] (which deploys and
     manages Ironic; see [how Metal3 uses Ironic][irso-ironic]
     for an architecture diagram), IPA
     (Ironic Python Agent, which runs on the host during
     provisioning and inspection), the Ironic TOR network
     configuration tool, the Ironic Prometheus metrics exporter,
     and cloud-init for post-provisioning configuration.
   - On the CAPI side, Metal3 is compatible with multiple
     bootstrap providers beyond kubeadm: the ignition bootstrap
     provider, the k3s bootstrap provider, and custom providers such as the fake
     kubeadm provider.
...
- Explain the design principles and best practices the project
  is following.
   - Kubernetes-native CRDs + controllers; declarative,
     vendor-neutral baremetal management using standard BMC
     ...
     (BMO, CAPM3, IPAM, IrSO).
   - The provisioner layer in BMO is abstracted behind a
     provisioner interface, so Ironic is the default but not
     the only possible backend; anything implementing the
     interface can serve as a provisioner. The
     PreprovisioningImage controller follows the same pattern
     for pre-provisioning image management.
   - The stack is intentionally modular: CAPI and CAPM3 are
     optional. Users can run BMO standalone to manage
     BareMetalHosts directly, consume BMHs with their own
     controllers, or combine CAPM3 with a non-kubeadm bootstrap
     provider. Everything above and below BMO is composable.
- Outline or link to the project's architecture requirements?
  ...
  Test and Production environments, as applicable.
   ...
   - Architecture documented in the
     [Metal3 project overview][arch].
- Define any specific service dependencies the project relies on
  in the cluster.
   - Kubernetes API server (with etcd), a provisioner backend
     (Ironic by default, deployed via the
     [Ironic Standalone Operator][irso], or any other
     implementation of BMO's provisioner interface deployed
     separately), and MariaDB or another SQL-compatible
     relational database in case the user opts to store
     provisioner state in an external database. PXE boot
     requires DHCP/TFTP infrastructure. No external database or
     SaaS dependency is required.
...
- Describe the project's High Availability requirements.
   - BMO and CAPM3 controllers support leader election via
     ...
     deployed in active-active HA configurations using the
...
- Describe the project's storage requirements, including its use
  of ephemeral and/or persistent storage.
   - Storage needs are primarily in Ironic and image-serving
     ...
     persistent external relational database (MariaDB/MySQL or a
     compatible derivative). In production, image-server storage for OS images
     (and IPA artifacts) is typically the dominant requirement and should be sized
...
- Please outline the project's API Design:
   - Describe the project's API topology and conventions
      - Metal3 uses Kubernetes CRDs following standard API
        ...
        IPPool, IPClaim, and IPAddress. When the
        [Ironic Standalone Operator][irso] is used, it also
        introduces the `Ironic` CRD for declarative Ironic
        deployment.
      ...
      - Controller grouping: BareMetalHost is reconciled by BMO
        ...
        Metal3 IPAM controllers; and the Ironic CRD is reconciled
        by the Ironic Standalone Operator (IrSO) when it is used.
   ...
   - Describe any new or changed API types and calls, including
     calls to cloud providers, that will result from this project
   ...
   - Describe versioning of any new or changed APIs, including
     how breaking changes are handled
      - Current released Metal3 APIs are mostly v1beta2 with BMH API at v1alpha1.
        API versions follow Kubernetes conventions (v1alpha1, v1beta1 etc).
        Unreleased `main` branch work towards newer Cluster API
  ...
  more complex integration and configuration?
   - Installation via `clusterctl init --infrastructure metal3`
    ...
    provisioning and Cluster API-based deployments.
     ...
     [Ironic Standalone Operator][irso]. BMO and IrSO are
     installed via kustomize overlays or static manifests
     directly. Metal3 does not currently provide Helm
     charts or a dedicated CLI for these components. The
...
- Please provide a link to the project's cloud native security
  self assessment.
   - [Security self-assessment][self-assessment] and
      ...
      - Metal3's recommended production setup uses Ironic
        ...
        RBAC. Users who need to loosen defaults can adjust:
         - **Widen RBAC scope**: granting broader permissions to
           controllers, which increases blast radius of credential
           leak.
   ...
   - Explain the least minimal privileges required by the project
     and reasons for additional privileges.
      ...
      - Additional privileges: default manifests are generally
        ...
        namespaced Roles/RoleBindings, reducing privilege scope
...
- Describe what project installation and configuration look like.
   - Typical bootstrap flow: create a bootstrap Kubernetes
     ...
     install IrSO via kustomize manifests, deploy Ironic via
     IrSO (unless Ironic is already provided externally), then
     deploy Bare Metal Operator manifests via kustomize.
...
- How can this project be enabled or disabled in a live cluster?
  ...
  nodes.
   - BMO, CAPM3, and IPAM are standard Kubernetes Deployments
     with no control plane downtime. Already-provisioned
     baremetal hosts continue running when controllers are
     removed, but all reconciliation and lifecycle operations
     stop. Controllers place finalizers on resources such as
     BareMetalHost and Metal3Machine, so deleting CRs while
     the controller is absent will hang until the finalizer
     is resolved. Operators should clean up managed resources
     before permanently uninstalling controllers, or scale
     the Deployment to zero for temporary disablement.
...
- How does the project clean up any resources created, including
  CRDs?
   - Controllers use finalizers on BareMetalHost and
     Metal3Machine resources so hardware is decommissioned
     before the resource record is removed; resources should
     therefore be deleted top-down and allowed to reconcile at
     each step. The documented sequence is:
      1. Delete the Cluster API resources, working down from
         `Machine`/`MachineDeployment` objects (workers first,
         then the control plane) and finally the `Cluster`
         object. Deleting a `Machine`/`MachineDeployment`
         deletes the associated `Metal3Machine` automatically.
      1. As the Metal3Machines are released, BMO deprovisions
         the corresponding BareMetalHosts and they return to
         the `available` state. When a BareMetalHost itself is
         deleted, its finalizer first cleans the host (removing
         partitioning tables from its disks) and powers it off
         (`deprovisioning` → `powering off before delete` →
         `deleting`) before the record is removed.
      1. Delete any remaining standalone BareMetalHost and IPAM
         (IPPool/IPClaim/IPAddress) resources and wait for the
         finalizers to complete.
      1. Remove the CAPM3 and IPAM provider components with
         `clusterctl delete --infrastructure metal3 --ipam metal3`.
         By default this removes only the provider components
         and leaves the CRDs (and their custom resources) in
         place; passing `--include-crd` also removes the CRDs,
         which force-deletes any remaining objects of those
         kinds, so it should only be used once the resources
         above are gone.
      1. Remove BMO and Ironic the same way they were installed,
         for example by deleting the BMO release manifests and
         the Ironic object.
...
- Describe how the project handles rollback procedures.
   - Rollback of Metal3 controller versions is not officially
     supported. Cluster API does not support version downgrades
     and warns that they may leave a management cluster
     non-functional. In practice, redeploying a previous
     controller version may work for patch-level changes within
     the same release line, but CRD schema changes or new
     status fields introduced by the newer version may cause
     the older controller to behave incorrectly.
...
- How can a rollout or rollback fail? Describe any impact to
  already running workloads.
   - CRD schema incompatibilities with stored resources, or new
     ...
     Already-provisioned baremetal hosts continue running
...
- Explain how the project informs users of deprecations and
  removals of features and APIs.
   - Through release notes, the [Metal3 book][book], the
     metal3-dev mailing list, and community meetings. API field
     deprecations follow the Kubernetes deprecation policy with
     at least one release of overlap.
- Explain how the project permits utilization of alpha and beta
  capabilities as part of a rollout.
   - Alpha features are gated behind feature flags in controller
     ...
     follow Kubernetes conventions (v1alpha1, v1beta1, v1beta2).
     Component maturity:

     | Component | Maturity | Version |
     | :---------- | :--------- | :-------- |
     | Baremetal Operator (BMO) | **Stable** | v0.14.x |
     | Cluster API Provider Metal3 (CAPM3) | **Stable** | v1.14.x |
     | IP Address Manager (IPAM) | **Stable** | v1.14.x |
     | Ironic Standalone Operator (IrSO) | **Beta** | v0.11.x |
     | Ironic Image | **Stable** | v38.0.0 |
     | MariaDB Image | **Archived** | No longer maintained |
...
- Describe which conditions enabling / using this project would
  ...
  sockets, inodes, etc.)
   - Extremely large fleet sizes (thousands of hosts and clusters) with
...
- Describe the load testing that has been performed on the
  project and the results.
   - Red Hat has validated Metal3 at 2000+ node scale in
     ...
     As a scalability experiment, Ironic has also been tested
     with simulated (fake IPA) agents at large scale.
- Describe the recommended limits of users, requests, system
  resources, etc. and how they were obtained.
   - A single BMO instance comfortably manages hundreds of
     BareMetalHosts; for 1000+ hosts, Ironic tuning
     ([conductor workers][ironic-conductor], database connection
     pooling) is recommended. Limits determined through adopter
...
- Describe the signals the project is using or producing,
  ...
  storage.
   - Produces Prometheus metrics (reconciliation duration, error
     ...
     default.
   - Ironic ships the Ironic Prometheus Exporter, which
     produces Prometheus-compatible metrics for Ironic
     baremetal operations, extending the observability
     surface beyond the Kubernetes controllers.
   - The BareMetalHost CR is designed with extensive status
     fields so the full state of managed physical machines is
     visible directly via the Kubernetes API without additional
     tooling.
   - Ironic can collect IPA (Ironic Python Agent) logs from the
     host during provisioning via its deploy-log backend. The
     retrieved logs are stored by Ironic (for example on the
     local filesystem) rather than emitted to the Ironic
     container's stdout/stderr, so they are not directly
     available as Kubernetes pod logs. Operators must configure
     and query Ironic's log storage to access IPA logs.
   - Hardware inspection captures a point-in-time snapshot of
     the hardware composition (CPU, RAM, disk, NIC) of each
     physical machine and stores it in HardwareData resources.
     This inventory reflects the state at the most recent
     inspection; without re-inspection it can become stale
     after hardware changes. Operators can trigger
     re-inspection to refresh the data.
   - BMO supports subscribing to hardware events emitted by the
     BMC through BMCEventSubscription resources (Redfish event
     subscriptions), allowing BMC-originated events (for example
     hardware health alerts) to be surfaced as an additional
...
- Describe any dashboards the project uses or implements as well
  as any dashboard requirements.
   - Metal3 does not ship built-in dashboards. For metrics
     dashboarding, Prometheus-compatible tools such as Grafana
     can scrape the Metal3 metrics endpoint to track
     BareMetalHost state distribution, provisioning latency,
     and error rates. For CR visualization, Headlamp with the
     [Metal3 plugin][headlamp-plugin] (initial alpha release)
     provides a Kubernetes-native UI for
     browsing and inspecting Metal3 resources directly.
- Describe how the project surfaces project resource requirements
  ...
  FinOps
   - The Metal3 controllers (BMO, CAPM3, IPAM) run as standard
     Kubernetes Deployments with defined resource
     requests/limits, so their own footprint (roughly
     256Mi–512Mi memory and ~0.5 CPU per controller, plus
     Ironic and its database storage that scales with fleet
     size) is attributable through standard Kubernetes cost
     tooling such as OpenCost/Kubecost by namespace and
     workload. In addition, BareMetalHost status exposes the
     managed hardware inventory (CPU, RAM, disk, NIC details)
     discovered during inspection, enabling adopters to build
     asset tracking and capacity planning for the managed fleet
     on top of the CRD data.
- Which parameters is the project covering to ensure the health
  of the application/service and its workloads?
   - Health is tracked across BMH, CAPM3, and IPAM resources:
     ...
     IPAddress allocation state. BMH also surfaces BMC health information in
     its status fields. Controller health is exposed via `/healthz` and `/readyz`
     endpoints, and monitored via liveness and readiness probes.
...
- Describe the specific running services the project depends on
  in the cluster.
   - Kubernetes API server, a provisioner backend (Ironic by
     default, or any other implementation of the provisioner
     interface), cert-manager by default for certificate
     management, and optionally DHCP/TFTP infrastructure for
     PXE boot. When using the default Ironic backend, Ironic
     itself relies on additional components: Apache HTTP server
     (for image serving), dnsmasq (for DHCP/PXE), iPXE
     firmware, a firmware builder, IPA (Ironic Python Agent),
     and optionally the Ironic TOR switch management components.
     Operators can bring their own certificates, but cert-manager
     is the default deployment path. All dependencies except
     Kubernetes can be deployed as containers within the
     management cluster.
...
- How does this project recover if a key component or feature
  ...
  database, leader node, etc.
   - Controllers use exponential backoff and requeue; if Ironic
     ...
     reconciliation retries automatically. Baremetal hosts
...
- Describe the known failure modes.
   ...
   - Firmware bugs causing inspection/provisioning failures, with documented
   ...
   - Resource contention during concurrent provisioning of
     large batches: Ironic has a finite conductor thread pool
     and BMO's work queue controls processing rate. The
     BareMetalHost state machine is idempotent and each host
     is reconciled independently, so concurrent operations do
     not race with each other, but throughput is bounded by
     Ironic capacity
...
- Describe how the project ensures alignment with CNCF
  recommendations for attribution notices.
   - The project follows CNCF guidance by providing a project
     LICENSE and SBOMs attached to released container images.
     See the
   ...
   - How does the project ensure its security reporting and
     ...
     (organizational and individual)?
      - The Metal3 [security team][sec-policy] includes members
        ...
        team has a designated Security Lead, and new members
        are added via a nomination process documented in
        [CONTRIBUTOR-ROLE.md][roles].
...
[arch]: https://book.metal3.io/project-overview.html
...
[self-assessment]: https://github.com/metal3-io/metal3-docs/blob/main/security/self-assessment.md
...
[irso-ironic]: https://book.metal3.io/ironic/introduction.html#how-metal3-uses-ironic
[ironic-conductor]: https://docs.openstack.org/ironic/latest/install/refarch/common.html#nodes-per-conductor
[headlamp-plugin]: https://github.com/headlamp-k8s/plugins/releases/tag/metal3-0.1.0-alpha
